...
if (process.versions.node !== toolchain.javascript.node) {
  throw new Error(`Datahike/DynamoDB artifact determinism requires Node ${toolchain.javascript.node}; running ${process.versions.node}`);
}
...
if (!new RegExp(`(?:openjdk|java) version "${escapeRegex(javaRelease)}"`, "iu").test(javaVersion)
  ...
  throw new Error(`Datahike/DynamoDB artifact determinism requires Java ${toolchain.jvm.java} (runtime ${javaRelease}+${javaBuild}); received ${firstLine(javaVersion)}`);
}

const clojureDescription = output("clojure", ["-Sdescribe"]);
if (!clojureDescription.includes(`:version "${toolchain.jvm.clojureCli}"`)) {
  throw new Error(`Datahike/DynamoDB artifact determinism requires Clojure CLI ${toolchain.jvm.clojureCli}`);
}

const first = await build();
const second = await build();
if (first.sha256 !== second.sha256 || first.bytes !== second.bytes) {
  throw new Error(`Datahike/DynamoDB Lambda builds differ: ${JSON.stringify({ first, second })}`);
}
console.log(`datahike-dynamodb-lambda\tsha256:${second.sha256}\t${second.bytes} bytes\tbyte-for-byte reproducible`);

async function build() {
  execFileSync("clojure", ["-T:build", "datahike-dynamodb-lambda"], {
    cwd: root,
    env: childEnvironment,
    stdio: "inherit",
  });
  const bytes = await readFile(path.join(root, "dist/datahike-dynamodb/function.jar"));
  return {
