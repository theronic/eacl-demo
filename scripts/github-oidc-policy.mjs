...
const ACTIVE_DEPLOYMENT_ACTIVATION = "every-production-push";
...
export function validateManifest(manifest) {
  ...
  assert(manifest.repository?.deploymentRef === "refs/heads/production", "deployment ref must be exactly the production branch");
  ...
  for (const authority of manifest.authorities) {
    ...
    if (authority.authorityClass === "ordinary-deployment") {
      ...
      assert(authority.activation === ACTIVE_DEPLOYMENT_ACTIVATION, `ordinary authority is not active on every production push: ${authority.id}`);
