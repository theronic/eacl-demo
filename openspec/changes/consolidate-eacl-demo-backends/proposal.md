...
- Add isolated read-only runtimes for:
  ...
  - Datomic Pro/DynamoDB using a temporary provisioning transactor, history-preserving storage for the later separately qualified EC2 demo, and a SnapStarted Lambda read-only Peer that serves all EACL consistency selections over the fixed database value captured at initialization without synchronization;
  - Datalevin/in-memory using approximately ten thousand deterministic resources in a 1024 MB managed Java Lambda whose ready native state is captured in a qualified published-version SnapStart snapshot;
...
- `datomic-read-only-demo`: One-million-resource Datomic/DynamoDB provisioning and a transactor-free SnapStarted Lambda serving every EACL consistency selection over one fixed database value.
- `datalevin-memory-demo`: Approximately ten-thousand-resource Datalevin in-memory Lambda at 1024 MB, qualified SnapStart restore, and honest ephemeral-storage lifecycle.
...
- **Known gates:** Both Datahike storage profiles must pass restored-reader qualification at 1024 MB, including post-restore storage access and memory headroom; Datalevin must pass the full restored semantic/load gate at 1024 MB; Datomic must qualify published-version SnapStart and every advertised consistency selection without `d/sync`. Jank is parked and, if later unparked, must build and smoke on Linux x86_64/AL2023 and cannot use Lambda SnapStart.
