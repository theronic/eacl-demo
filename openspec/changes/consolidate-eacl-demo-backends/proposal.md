...
- Continuously deploy every registered demo when a commit reaches `theronic/eacl-demo:demos`. Each run uses that exact demo commit and the exact EACL Core revision pinned by it, then starts independent profile jobs immediately with no fleet-wide barrier, fail-fast cancellation, or atomic multi-profile rollout.
- Keep merge deployments deliberately short: build/package, immutable candidate deployment, bounded health/bootstrap/identity/allowed/denied/mutation-denial smoke probes, then per-profile alias promotion or rollback. Formal verification, full conformance, fault campaigns, load sweeps, data seeding, and stateful migrations are not merge-deployment gates.
...
- **Rollout:** profiles and workflow runs are intentionally uncoordinated and may expose different generations. Each descriptor exposes its actual source/deployment identity; a failed profile keeps its previous healthy deployment rather than blocking or rolling back the fleet.
- **Known gates:** Datahike/DynamoDB still requires adapter and real-AWS repair evidence; Datalevin still requires native/SnapStart lifecycle evidence; Datomic storage retains history but its read-only Lambda supports only its fixed current snapshot; Jank must build and smoke on Linux x86_64/AL2023 and cannot use Lambda SnapStart.
