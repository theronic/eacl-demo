...
Every `demos` merge SHALL build/package each registered profile, deploy an immutable candidate, and run bounded health, bootstrap/identity, one allowed exemplar, one denied exemplar, and public-mutation-denial probes. It SHALL NOT wait for formal verification, full conformance, fault campaigns, load sweeps, browser suites, or other profiles.
