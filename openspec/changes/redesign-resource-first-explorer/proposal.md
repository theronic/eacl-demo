...
- Keep prominent counts, their latency and cache result immediately together beside each lookup, count, permission decision, and reverse lookup. Do not add a separate timing column, global query-evidence strip, query-activity dashboard, or browser-only resource search. Retain the original cache controls and diagnostics.
