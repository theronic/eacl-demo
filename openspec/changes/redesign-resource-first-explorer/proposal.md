...
The subsequent control revision places Seed Data beside the navbar object count, moves the theme control before the rightmost View As, and omits inventory timings from the navbar. Page Size replaces the density icon in the tree toolbar. Backend radios use the same plain treatment as Consistency Mode, whose panel uses the standard background. Schema source precedes inferred types and starts expanded. Check Permission re-runs on input changes. Seeding progress remains visible while backend selection stays enabled, and UI updates preserve viewport position.
