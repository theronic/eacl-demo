...
`npm run build:datascript-runtime` fetches the exact EACL Core SHA recorded in `dependencies/eacl-core.lock.json`, prepares that source, and compiles `eacl-demo.datascript.runtime` with `cljs.main -t browser -O advanced`. This is a normal in-page ClojureScript build. It does not create a Web Worker, Blob, worker protocol, or server process.
