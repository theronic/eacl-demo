...
export function CanPermissionFooter(
  props: CanPermissionFooterProps,
): JSX.Element {
  const [expanded, setExpanded] = createSignal(
    !matchMedia("(max-width: 800px)").matches,
  );
  const [subjectType, setSubjectType] = createSignal(
    props.initial.subject.type,
  );
  const [subjectId, setSubjectId] = createSignal(props.initial.subject.id);
  const [resourceType, setResourceType] = createSignal(
    props.initial.resource.type,
  );
  ...
  const permissions = createMemo(
    () => props.permissionsByType[resourceType()] ?? [],
  );
  const subjectIds = createMemo(() =>
    props
      .subjects()
      .filter((subject) => subject.type === subjectType())
      .map((subject) => subject.id),
  );
  const resourceIds = createMemo(() =>
    props
      .resources()
  ...
  const execute = async () => {
    if (!valid()) {
      ...
      setError(
        new Error(
          "Choose a subject, permission, and resource before querying.",
        ),
      );
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div class="can-permission-footer__query" hidden={!expanded()}>
        <span class="can-permission-footer__group">
          ...
          <select
            ...
            value={subjectType()}
            onChange={(event) => setSubjectType(event.currentTarget.value)}
          >
            <For each={props.subjectTypes}>
      ...
      <Show when={error()}>
        <p class="can-permission-footer__error" role="alert">
          {errorMessage()}
        </p>
      </Show>
    </aside>
