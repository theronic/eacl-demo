...
DataScript has an additional trust chain before bootstrap. The shell verifies one exact content-addressed profile publication, fetches only the corresponding same-origin full-digest worker path under a bounded no-redirect/no-credential policy, hashes the received bytes, and executes those verified bytes through a revoked Blob URL. An initialize message then binds the lifecycle to the publication's exact profile, demo/Core commits, worker digest, deployment ID, and fixture digest. Normal operations are rejected before initialization, and every response repeats the exact identity and request/operation correlation.

Strict runtime validators compile the JSON schemas for client requests, server responses, worker messages, fixture manifests, descriptors, registries, and release manifests. They reject unknown fields and malformed identities without placing rejected values in thrown errors or logs.
...
Server cursors are HMAC-SHA-256 authenticated, byte-bounded, and expire. Their signed scope includes contract, profile, deployment, data manifest, lifecycle, operation, and a canonical query digest. DataScript instead issues opaque random tokens backed by a bounded worker-lifecycle registry; the registry entry binds the closed query scope and offset, and reset or worker replacement destroys it. Neither form exposes raw query values or crosses an incompatible identity/lifecycle; invalid/expired and scope-mismatched cursors use distinct stable errors.
