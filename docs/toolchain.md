...
`toolchain.json` records the currently tested tool versions. Version-manager
files mirror it for developer convenience; they are not a second dependency
certification system.

The JavaScript baseline is Node 26.8.1 and its bundled npm 11.19.0. JVM builds
use Eclipse Temurin semver 25.0.4+101.0.LTS (the patched runtime reports
25.0.4.1+1-LTS), Clojure 1.12.5, Clojure CLI 1.12.5.1664, and ClojureScript
1.12.145. Infrastructure is raw CloudFormation validated with the versions
recorded in `toolchain.json`.
...
An update changes the package-manager inputs and any applicable version-manager
mirrors. It does not require a duplicate lock or certification manifest.
