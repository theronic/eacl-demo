...
`node scripts/verify-static-artifact-determinism.mjs` is the separate material-change check for the real static deployment artifact. It builds the main Vite entry, DataScript Vite entry, advanced-compiled ClojureScript worker, and assembled upload tree twice, then compares every output byte and verifies that the worker manifest, compiled worker, content-addressed filename, and assembled copy carry the same digest. Both checks refuse to run under a Node version other than the exact pin in `toolchain.json`. The compiled check is manual qualification and is intentionally outside the fast ordinary `demos` deployment path.
