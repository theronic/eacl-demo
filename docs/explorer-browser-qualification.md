...
This qualification is deliberately outside the ordinary `demos` push workflow. It runs only through `.github/workflows/qualify-explorer.yml` manual dispatch or the equivalent local command. Its result may qualify a staged shell or material shell/contract change, but it cannot block or trigger a normal demo deployment.
...
The separate DataScript Chromium qualification publishes an enabled test-only DataScript profile bound to the actual compiled worker digest and locked Core SHA. It verifies the targeted publication fetch and full-digest worker fetch, the publication-to-byte-to-Blob-to-initialize identity chain, and the reported demo/Core/artifact/data/basis facts. After readiness it clears the request log, then exercises subjects, object lookup, outbound and reverse traversal, pagination controls, schema, cache, and allowed/denied authorization through the shared explorer. Every post-readiness operation must remain inside the worker and produce no network request.
