...
(defn initialize-runtime!
  ...
  initialization. Datomic SnapStart is deliberately disabled so the Peer can
...
(defn initialize
  ...
  ([environment open-reader!]
   (let [{:keys [reader-config identity memory-mib]} (parse-environment environment)
