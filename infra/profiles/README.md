...
versioned artifact, an IAM-only Function URL alias, fixed concurrency one,
...
IAM-only and SnapStart is explicitly off until restore qualification.

`datomic-dynamodb-runtime.yaml` packages one Java 25/x86_64 fixed-current
candidate from an immutable S3 object version. Its Function URL is IAM-only and
bound to the candidate alias. SnapStart is explicitly off until the required
...
version, nonnumeric alias, and IAM-only alias-qualified Function URL, but that
