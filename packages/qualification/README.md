...
This package drives the same profile contract through local transports, directly authorized staged origins, and the staged CloudFront route. Reports retain exact profile, demo, EACL Core, artifact, deployment, and data identities; authorization headers and credentials are never serialized.
...
Initial enablement uses a fail-closed gate. A profile remains non-enabled unless a staged CloudFront run covers every common qualification category, exact report/descriptor/deployment identities agree, representative workloads pass, the tested transport is released, the evidence postdates the immutable deployment, and a content-addressed observability-readiness record matches the same deployment. That record requires bounded structured/redaction-audited logs, the closed runtime signal set, profile/resource-scoped action-enabled SNS-to-Telegram alarms in `OK`, a ready dashboard and runbook, and passing health/bootstrap/exemplar synthetics. Every synthetic must traverse the staged CloudFront profile route after deployment and observe the exact immutable identity. A boolean readiness flag is rejected. The gate's content-addressed evidence ID binds the deployment, qualification, workload, and observability records and never mutates the input registry.
