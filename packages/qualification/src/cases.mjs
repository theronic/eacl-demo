...
function relationshipCase() {
  return operationCase("relationship-filter-and-shape", "relationship", ["list-relationships"], async ({ transport, descriptor }) => {
    const page = successfulData(await transport.request("list-relationships", { resourceType: "account", resourceId: "account-0", relation: "owner", pageSize: 25, consistency: advertisedConsistency(descriptor) }), "list-relationships");
    assertPage(page, 25);
...
function reverseRelationshipCase() {
  return operationCase("reverse-relationship-discovery", "relationship", ["reverse-relationships"], async ({ transport, descriptor }) => {
    const page = successfulData(await transport.request("reverse-relationships", { subjectType: "user", subjectId: "user-1", relation: "owner", pageSize: 25, consistency: advertisedConsistency(descriptor) }), "reverse-relationships");
    assertPage(page, 25);
...
function demandInput(demand, descriptor) {
  return { subjectType: demand.subject.type, subjectId: demand.subject.id, resourceType: demand.resource.type, resourceId: demand.resource.id, permission: demand.permission, consistency: advertisedConsistency(descriptor) };
}

function advertisedConsistency(descriptor) {
  const mode = descriptor?.capabilities?.consistencyModes?.[0];
  if (!CONSISTENCY_MODES.includes(mode)) throw new Error("descriptor has no advertised consistency mode");
  return mode;
}
