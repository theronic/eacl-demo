...
test("canonical parsing and serialization retain only bounded semantic intent", () => {
  ...
  assert.deepEqual(parsed.state, { backend: "datahike", storage: "dynamodb", platform: "lambda-1769", subjectType: "user", subject: "user:alice", resourceType: "server", resourceId: "server-42", permission: "read", relation: "owner", view: "authorization", pageSize: 50, cacheEnabled: false, consistencyMode: "exact" });
...
test("cursor, token, basis, request, revision, and secret fields never serialize", () => {
  ...
  assert.equal(parsed.canonicalSearch, "?backend=datahike&storage=s3&platform=lambda-1769");
  const serialized = serializeCanonicalUrl({ ...parsed.state, cursor: "opaque", token: "x", basis: "42", requestId: "r", secret: "bad" }, catalog);
  assert.equal(serialized, "?backend=datahike&storage=s3&platform=lambda-1769");
});
...
test("preference-shaped portable fields normalize strictly", () => {
  ...
  assert.equal(serializeCanonicalUrl({ ...parsed.state, pageSize: 1000, cacheEnabled: true, consistencyMode: "minimize" }, catalog), "?backend=datomic&storage=dynamodb&platform=lambda-1769&page-size=1000&cache=on&consistency=minimize");
});
...
test("platform links fail closed to the supported deployment matrix", () => {
  ...
  assert.equal(unsupported.state.platform, "lambda-1769");
  assert.deepEqual(unsupported.issues, [{ code: "invalid-platform", field: "platform" }]);
  const datalevin = parseCanonicalUrl("?backend=datalevin&storage=embedded&platform=ec2", catalog);
  assert.equal(datalevin.state.platform, "ec2");
  assert.deepEqual(datalevin.issues, []);
});

test("legacy 1 GiB Lambda links migrate without presenting an invalid-link error", () => {
  const migrated = parseCanonicalUrl("?backend=datomic&storage=dynamodb&platform=lambda-1024", catalog);
  assert.equal(migrated.state.platform, "lambda-1769");
  assert.equal(migrated.canonicalSearch, "?backend=datomic&storage=dynamodb&platform=lambda-1769");
