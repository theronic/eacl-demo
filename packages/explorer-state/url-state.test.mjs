...
test("canonical parsing and serialization retain only bounded semantic intent", () => {
  ...
  assert.deepEqual(parsed.state, { backend: "datahike", storage: "dynamodb", subjectType: "user", subject: "user:alice", resourceType: "server", resourceId: "server-42", permission: "read", relation: "owner", view: "authorization", pageSize: 50, cacheEnabled: false, consistencyMode: "exact" });
...
test("cursor, token, basis, request, revision, and secret fields never serialize", () => {
  ...
  assert.equal(parsed.canonicalSearch, "?backend=datahike&storage=s3");
  const serialized = serializeCanonicalUrl({ ...parsed.state, cursor: "opaque", token: "x", basis: "42", requestId: "r", secret: "bad" }, catalog);
  assert.equal(serialized, "?backend=datahike&storage=s3");
});
...
test("individual values and the total search string are bounded", () => {
  ...
  assert.deepEqual(totalResult.state, { backend: "datahike", storage: "s3" });
});
...
test("preference-shaped portable fields normalize strictly", () => {
  ...
  assert.equal(serializeCanonicalUrl({ ...parsed.state, pageSize: 1000, cacheEnabled: true, consistencyMode: "current" }, catalog), "?backend=datomic&storage=dynamodb&cache=on&consistency=current");
});
