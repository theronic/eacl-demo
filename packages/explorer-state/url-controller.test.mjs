...
test("direct links normalize malformed, duplicate, unknown, and incompatible fields", () => {
  ...
  assert.deepEqual(states[0].state, { backend: "datomic", storage: "dynamodb", platform: "lambda-1769" });
  assert.deepEqual(states[0].issues.map(({ code }) => code).sort(), ["duplicate-field", "invalid-storage", "invalid-value", "unknown-field"].sort());
  assert.equal(browser.location.search, "?backend=datomic&storage=dynamodb&platform=lambda-1769");
...
test("backend replacement chooses compatible storage in one history entry", () => {
  ...
  assert.equal(browser.location.search, "?backend=jank&storage=memory&platform=lambda-1769&subject=user%3Aalice");
